...
class EnhancedAuthService {
  ...
  async checkDuplicate(
    ...
    try {
      ...
      const response = await fetch('http://13.204.65.158/api/operators/check-duplicate', {
        method: 'POST',
        headers: {
  ...
  async login(
    ...
    try {
      ...
      const deviceId = await this.getDeviceId();
      const response = await fetch('http://13.204.65.158/api/operators/login', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async logout(examId?: string): Promise<boolean> {
    try {
      ...
      try {
        await fetch('http://13.204.65.158/api/operators/logout', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  async refreshToken(): Promise<boolean> {
    try {
      ...
      const response = await fetch('http://13.204.65.158/api/operators/refresh-token', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${session.token}`,
